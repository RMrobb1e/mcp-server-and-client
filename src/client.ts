...
async function main() {
  ...
  const [{ tools }, { prompts }, { resourceTemplates }, { resources }] =
    await Promise.all([
      mcp.listTools(),
      mcp.listPrompts(),
  ...
  mcp.setRequestHandler(CreateMessageRequestSchema, async (request) => {
    const texts: string[] = [];
    for (const messages of request.params.messages) {
      const text = await handleServerMessagePrompt(messages);
  ...
  while (true) {
    const option = await select({
      ...
      choices: ["Tools", "Query", "Prompts", "Resources", "Exit"],
    });
...
async function handleQuery(tools: Tool[]) {
  ...
  console.log(
    // @ts-expect-error
    text || toolResults[0]?.result?.content[0]?.text || "No response",
  );
}
